...
import { fetchCoursesData, fetchDeptData } from '../../store/actions/dataActions';
				...
				{Register && (
					<View style={{ flex: styles.listText.flex / 1.5, marginLeft: 0, alignItems: 'flex-start' }}>
						<TouchIcon
							...
							toggleIcons={['square-outline', 'checkbox-outline']}></TouchIcon>
					</View>
				)}
				<Text style={{ ...styles.listText, textAlign: 'left', color: Colors.primary, fontSize: 15 }}>
					{courseCode}
				</Text>
				<Text numberOfLines={2} style={{ ...styles.listText, flex: 3, textAlign: 'left' }}>
					{courseTitle}
				</Text>
				<Text style={{ ...styles.listText, color: Colors.accent2, textAlign: 'center', fontSize: 15 }}>
					{creditUnits} Units
				</Text>
			</Touch>
		</View>
		...
		<View style={{ backgroundColor: Colors.switchPrimary }}>
			<View
				style={{
					...
					borderBottomWidth: !showSection ? 0.8 : 0,
				}}>
				<View
					style={{
					...
					<Text onPress={showSectionHandler.bind(this, title)} style={styles.sectionHeaderLabel}>
						{title}
					</Text>
					...
					<TouchIcon
						...
						color={Colors.switchWhite}
						toggleIcons={['arrow-dropdown', 'arrow-dropright']}></TouchIcon>
				</View>
			</View>
			...
			{showSection &&
				courses.length === 0 && ( // && !hiddenSections.includes(title)
					<View
						style={{
...
const CoursesScreen = ({ navig, source, submitCourses }) => {
	...
	for (let i = 1; i <= deptLevels.length; i++) {
		dataArr.push({
			...
			data: courses.filter((c) => {
				return c.courseLevel === parseInt(levels[i - 1]);
			}),
		});
	}
	...
	const renderItem = ({ item, index, section: { title, data } }) => {
		...
		return showingSection !== title ? ( // hiddenSections.includes(title) ?
			<></>
		) : (
