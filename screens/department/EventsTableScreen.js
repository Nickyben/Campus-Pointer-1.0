...
import {
  StyleSheet, Text, View, FlatList, Image, Platform, Modal
} from 'react-native';
      ...
      <View style={styles.infoContainer}>
        <View style={styles.titleContainer}>
          ...
          <View>
            ...
            {type && <Text style={styles.title2} numberOfLines={2}> {type} </Text>}
          </View>
          <View>
            {date && <Text style={styles.detail} numberOfLines={2}> {date} </Text>}
            {time && <Text style={styles.detail2} numberOfLines={2}> {time} </Text>}
          </View>

        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: addCalendar ? 'space-between' : 'flex-end', width: '100%' }}>
          {addCalendar &&
            <TouchIcon
              name={'calendar'}
              size={25}
...
const EventsTableScreen = ({ navig, source: { option } }) => {
  ...
  const eventsData = useSelector(state => state.dataReducer.availableEvents)
    .filter(event => event.department === 'Computer Engineering');
            ...
            <View style={styles.calendarModal}>
              <View style={styles.calendarContainer}>
                <View style={styles.calendarHeader}>
                  <Text style={{ color: '#fff', fontSize: 17, textAlign: 'center', fontFamily: 'OpenSansBold' }}>
                    Calendar
                  </Text>
                </View>
