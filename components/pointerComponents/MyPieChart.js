    ...
    <_View style={styles.container}>
      ...
      <_View style={styles.chartContainer}>
        <PieChart
          ...
          hasLegend={false}
        />
        <_View style={{ alignItems: 'center', padding: 10, paddingHorizontal: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between' }}>
          <_View style={{ flex: 1, padding: 5, paddingRight: 10, borderRightWidth: 1, borderColor: '#f3f6f7' }}>
            {data.filter((item, ind) => ind < data.length / 2).map((item, index) => {
              return (
                <_View key={index} style={{ flex:1,alignItems: 'center', marginBottom: 5, flexDirection: 'row', justifyContent: 'space-around' }}>
